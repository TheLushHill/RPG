...
/*:
 * @plugindesc [三烷]截屏系统
 * @author triacontane三十烷烷 汉化:硕明云书
 *
 ...
 * @text Ctrl键联用
 * @desc 使用各功能时是否需要同时按下Ctrl键。当目标密钥与其他插件冲突时使用。
 * @default false
 * @type boolean
 *
 * @param SimultaneousAlt
 * @text Alt键联用
 * @desc 使用各功能时是否需要同时按下Alt键。当目标密钥与其他插件冲突时使用。
 * @default false
 * @type boolean
 *
 * @param FileName
 ...
 * @param Location
 * @text 文件夹名称
 * @desc 文件的输出路径。提供相对路径和绝对路径。
 * 请用"/"或"\"指定分隔符。
 * @default captures
 *
 * @param FileFormat
 * @text 默认保存格式
 * @desc 图像的默认保存格式(png/jpeg/webp)
 * @default png
 * @type select
 * @option png
 * @option jpeg
 ...
 * @text 图像文件名
 * @desc 保存时写入图像右下角的著名图像文件名。「img/pictures」，无需扩展名
 * @default
 * @require 1
 * @dir img/pictures/
 * @type file
 ...
 * @param SeName
 * @text 间隔效果音
 * @desc 定期执行捕捉的间隔（秒单捕捉执行时演奏的效果音的文件名。
 * @default
 * @require 1
 * @dir audio/se/
 * @type file
 ...
 * @param JpegQuality
 * @text 输出质量
 * @desc JPEG输出时的质量。减小值时，容量也变小。(1...10)
 * @default 9
 * @type number
 * @min 1
 * @max 10
 *
 * @help 
 * 捕捉游戏中的游戏画面，可以将其保存到文件中，也可以将其显示为图片。
 * 捕捉将在以下时间执行 。
 *
 * 注意！
 * 无法保存捕捉图片的显示状态。
